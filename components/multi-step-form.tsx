...
import { ChevronRight, ChevronLeft, Check, User, Calendar, MapPin, Users, Sparkles } from "lucide-react"
...
export function MultiStepForm({ form, setForm, onSubmit, onBack }: MultiStepFormProps) {
  ...
  return (
    <>
      <div className="space-y-6 flex flex-col min-h-0 sm:min-h-0 relative">
        {/* Progress Steps */}
        ...
        {/* Step Content */}
        <Card className="bg-black/40 backdrop-blur-sm border-white/10 flex-1 flex flex-col relative overflow-visible mb-20 sm:mb-0">
          <CardContent className="p-6 sm:p-8 flex-1 flex flex-col relative overflow-visible min-h-0">
            {/* Step 1: Name */}
            {currentStep === 1 && (
...
// Navigation buttons component - separate to be fixed to viewport
export function FormNavigation({
  ...
  return (
    <div className="fixed sm:relative bottom-0 left-0 right-0 sm:inset-auto bg-black sm:bg-transparent border-t border-white/10 sm:border-t-0 z-[100] sm:z-auto shadow-[0_-4px_20px_rgba(0,0,0,0.8)] sm:shadow-none sm:mt-6">
      {/* Mobile: Back button - same size as Next but no background/border */}
      <div className="sm:hidden px-4 pt-4 pb-2">
        <button
          type="button"
          onClick={onBack}
          className="text-white/70 hover:text-white transition-colors flex items-center justify-center gap-2 h-14 w-full text-base font-medium bg-transparent border-none outline-none"
        >
      ...
      <div className="sm:hidden px-4 pb-4">
        {currentStep < totalSteps ? (
          <Button
            type="button"
            onClick={onNext}
            className="bg-white text-black hover:bg-white/90 h-14 w-full text-base font-medium"
          >
            Next
      ...
      <div className="hidden sm:flex container mx-auto max-w-3xl px-0 justify-between gap-4 pt-6 pb-0">
        <Button
          type="button"
          variant="outline"
          onClick={onBack}
